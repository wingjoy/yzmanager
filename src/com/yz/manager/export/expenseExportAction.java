...
public class expenseExportAction extends ActionSupport {
		...
		try {
			...
			WritableWorkbook book=Workbook.createWorkbook(outf);
			 ...
			 if(pw.isSystemManager()){
				 ex=expenseDao.selectExpense();	
			 }else if(pw.isDepartmentManager()){
				 ex=expenseDao.selectExpenseByDepartment(us.getDepartment());	
			 }else{
				 ex=expenseDao.selectExpenseByUserName(us.getUserName());	
			 }
