...
<body bgcolor="#E4FAF9">
 <%  
    ...
    expenseCount=expenseDao.selectClassExpenseCount(user.getDepartment(),systemName,String.valueOf(CurrentDate.getCurrentYear()),"报销","0");
                   ...
                   for(int k=0;k<dsc.size();k++){           
			          if(dsc.get(k).getFirstCName().equals(String.valueOf(dfc.get(j).getId()))){
			             out.println(
				              "<tr  height='25'>");
				              if(k==0){
				              		out.println( "<td align='center' width='7%' rowspan='"+dsc.size()+dfc.size()+1 +"'>"+sdp+"<br>(所有公司)</td>");
				              }
				              if(l==0){
					              out.println(
