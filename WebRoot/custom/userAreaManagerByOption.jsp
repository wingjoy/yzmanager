...
<body bgcolor="#E4FAF9">
 <% 
     String cdp=(String)session.getAttribute("cdp");
     ...
     if(request.getAttribute("cs")!=null){
	     cs=(List<userArea>)request.getAttribute("cs");
        ...
        currentPage=Integer.valueOf(request.getParameter("currentPage")).intValue();
        int total=daoUtil.selectUserAreaSizeByOption(cdp,uname);
        pg=new PageSet(total,15);  
        totalsize=pg.getTotalsize();
        totalPage=pg.getTotalpage();
        cs=daoUtil.selectUserArea(cdp,uname,currentPage,pg.getPagesize());
     }  
  %>
       ...
       <table class="tablelink" align="center">
           <tr align="right">
             ...
             <td><a class="tablelink" href="/custom/userAreaManagerByOption.jsp?currentPage=1">首页</a></td>
             <td><a class="tablelink" href="/custom/userAreaManagerByOption.jsp?currentPage=<%=pg.searchCurrentPage(currentPage-1) %>">上一页</a></td>
             <td><a class="tablelink" href="/custom/userAreaManagerByOption.jsp?currentPage=<%=pg.searchCurrentPage(currentPage+1)%>">下一页</a></td>
             <td><a class="tablelink" href="/custom/userAreaManagerByOption.jsp?currentPage=<%=totalPage %>">尾页</a></td>
             <td>跳转到第<select name="selectPage" onchange="document.location.href=this.value">           
             <%
                for(int j=1;j<=pg.getTotalpage();j++){
                if(j==currentPage){
                out.println(
                  "<option  selected value='/custom/userAreaManagerByOption.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }else {
                 out.println(
                  "<option value='/custom/userAreaManagerByOption.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }
              }   
              %>           
             </select>页</td>
