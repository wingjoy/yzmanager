...
<body bgcolor="#E4FAF9">
 <%
     ...
     if(request.getAttribute("cs")!=null){
	     cs=(List<customArea>)request.getAttribute("cs");
        ...
        currentPage=Integer.valueOf(request.getParameter("currentPage")).intValue();
        int total=daoUtil.selectCustomAreaSizeByOption(cdp);
        pg=new PageSet(total,15);  
        totalsize=pg.getTotalsize();
        totalPage=pg.getTotalpage();
        cs=daoUtil.selectCustomArea(cdp,currentPage,pg.getPagesize());
     }  
  ...
  %>
       ...
       <table class="tablelink" align="center">
           <tr align="right">
             ...
             <td><a class="tablelink" href="customAreaByOption.jsp?currentPage=1">首页</a></td>
             <td><a class="tablelink" href="customAreaByOption.jsp?currentPage=<%=pg.searchCurrentPage(currentPage-1) %>">上一页</a></td>
             <td><a class="tablelink" href="customAreaByOption.jsp?currentPage=<%=pg.searchCurrentPage(currentPage+1)%>">下一页</a></td>
             <td><a class="tablelink" href="customAreaByOption.jsp?currentPage=<%=totalPage %>">尾页</a></td>
             <td>跳转到第<select name="selectPage" onchange="document.location.href=this.value">           
             <%
                for(int j=1;j<=pg.getTotalpage();j++){
                if(j==currentPage){
                out.println(
                  "<option  selected value='customAreaByOption.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }else {
                 out.println(
                  "<option value='customAreaByOption.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }
              }   
              %>           
             </select>页</td>
