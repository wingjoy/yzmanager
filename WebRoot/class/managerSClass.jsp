 ...
 <%
  ...
  if(user==null) response.sendRedirect("../index.jsp"); 
   int totalsize=0;
     ...
     pg=new PageSet(daoUtil.selectSecondClassSize(),10);  
     totalsize=pg.getTotalsize();
     totalPage=pg.getTotalpage();
     currentPage=Integer.valueOf(request.getParameter("currentPage")).intValue();
	  List<secondClass> sc=new ArrayList<secondClass>();
	 sc=daoUtil.selectSecondClass(currentPage,pg.getPagesize());
      ...
      <table class="left-font01" width="40%" align="center" border="0" cellspacing="0" cellpadding="0" >
       ...
       <table class="tablelink" width="60%" align="center">
           <tr align="right">
             ...
             <td><a class="tablelink" href="managerSClass.jsp?currentPage=1">首页</a></td>
             <td><a class="tablelink" href="managerSClass.jsp?currentPage=<%=pg.searchCurrentPage(currentPage-1) %>">上一页</a></td>
             <td><a class="tablelink" href="managerSClass.jsp?currentPage=<%=pg.searchCurrentPage(currentPage+1)%>">下一页</a></td>
             <td><a class="tablelink" href="managerSClass.jsp?currentPage=<%=totalPage %>">尾页</a></td>
             <td>跳转到第<select name="selectPage" onchange="document.location.href=this.value">           
             <%
                for(int j=1;j<=pg.getTotalpage();j++){
                if(j==currentPage){
                out.println(
                  "<option  selected value='managerSClass.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }else {
                 out.println(
                  "<option value='managerSClass.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }
              }   
              %>           
             </select>页</td>
