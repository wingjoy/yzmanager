...
<body bgcolor="#E4FAF9">
 <%  
     ...
     System.out.println("aaaaa:"+collect +"---"+user.getDepartment()+"---"+systemName);
    int scount=daoUtil.selectSecondClassSize(collect,user.getDepartment(),systemName);
    int inStoreCount [][]=new int[scount][12];
    inStoreCount=storeHouseDao.selectInStoreCount1(user.getDepartment(),user.getDepartment(),"0",systemName,String.valueOf(CurrentDate.getCurrentYear()));
                   ...
                   for(int k=0;k<dsc.size();k++){         
			          if(dsc.get(k).getFirstCName().equals(String.valueOf(dfc.get(j).getId()))){
			             out.println(
				              "<tr  height='25'>");
				              if(k==0){
				              		out.println( "<td align='center' width='7%' rowspan='"+dsc.size() +"'>"+sdp+"<br>库房</td>");
				              }
				               if(s==0){
