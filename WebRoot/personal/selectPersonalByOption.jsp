...
<body  bgcolor="#E4FAF9">
 <%
     ...
     ps=(List<personal>)request.getAttribute("ps");
     ...
     currentPage=Integer.valueOf(request.getParameter("currentPage")).intValue();
        ...
        currentPage=Integer.valueOf(request.getParameter("currentPage")).intValue();
        int total=personalDao.selectPsByOptionInt(d1,u1,fc1,sc1,db,de,s1,sr);
        pg=new PageSet(total,15);  
        totalsize=pg.getTotalsize();
        totalPage=pg.getTotalpage();
        ps=personalDao.selectPsByOption(d1,u1,fc1,sc1,db,de,s1,sr,currentPage,pg.getPagesize());
     }
   ...
   <s:form name="myform" action="selectPersonalByOption?currentPage=1" method="post" theme="simple" >
      <table class="left-font01" align="center" border="0" cellspacing="0" cellpadding="0" >
         ...
         <td>搜索:</td>
           <td>
            <select name="select"  style="width:100px;" size="1"> 
			           ...
			           <option selected value="2">手机号</option> 
			           <option selected value="3">座机号</option> 
			           <option selected value="4">单位名称</option> 
			           <option selected value="5">单位地址</option>
			           <option selected value="6">备注</option>  
           </select>
        ...
        <td>&nbsp;&nbsp;添加日期:</td>
           <td>
              ...
              <td> &nbsp;&nbsp;<s:submit style="font-size:14px" name="submit" value="查 找"></s:submit>  </td>                     
                             <td> &nbsp;<a class="left-font01" href="personalExportByOptionAction.action">联系人导出</a></td>   
         </tr>
       <tr><td>&nbsp;</td></tr>
        </table>
      </s:form>
       ...
       <table class="left-font01" width="100%"  align="center" border="1" cellspacing="0" cellpadding="0" >
          ...
          <th>序号</th><th>添加日期</th><th>联系人</th><th>联系电话</th><th>单位</th><th>单位地址</th><th>备注</th><th>创建部门</th><th>创建人</th><th>详情</th><th>删除</th><th>修改</th>
          </tr>
          <%
           int i=1;
            for(personal a : ps){
         ...
         <table class="tablelink">
           <tr align="right">
             ...
             <td><a class="tablelink" href="selectPersonalByOption.jsp?currentPage=1">首页</a>&nbsp;</td>
             <td><a class="tablelink" href="selectPersonalByOption.jsp?currentPage=<%=pg.searchCurrentPage(currentPage-1) %>">上一页</a>&nbsp;</td>
             <td><a class="tablelink" href="selectPersonalByOption.jsp?currentPage=<%=pg.searchCurrentPage(currentPage+1)%>">下一页</a>&nbsp;</td>
             <td><a class="tablelink" href="selectPersonalByOption.jsp?currentPage=<%=totalPage %>">尾页</a>&nbsp;</td>
             <td>跳转到第<select name="selectPage" onchange="document.location.href=this.value">           
             <%
                for(int j=1;j<=pg.getTotalpage();j++){
                if(j==currentPage){
                out.println(
                  "<option  selected value='selectPersonalByOption.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }else {
                 out.println(
                  "<option value='selectPersonalByOption.jsp?currentPage="+j+"'>&nbsp;&nbsp;"+j+"&nbsp;&nbsp;</option>");
                }
              }   
              %>           
             </select>页</td>
